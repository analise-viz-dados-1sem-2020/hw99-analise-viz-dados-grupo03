...
#  pivot_longer(-Country.Name, -Country.Code, -Indicator.Name, -Indicator.Code, names_to = "Ano", values_to = "Valores")
```
...
df %>%
  ...
  filter(Indicator.Name == "NOME DO INDICADOR - SÓ COPIAR O QUE EU COLOQUEI") %>%
  filter(Country.Code = "BRA" + "LCN" + "WLD") %>%
  #ggplot (x = Ano, y = Valores) - Brasil, América Latina e Mundo 5>%
...
df %>%
  ...
  filter(Country.Code = "BRA" + "LCN" + "WLD") %>%
  ggplot (aes(x = Ano, y = Valores) + geom_point()) %>%
  facet_wrap(~Country.Name)
